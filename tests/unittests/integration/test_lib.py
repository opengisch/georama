...
class TestQgisProjectFileStructure:
    ...
    def test_qgis_project_collection_return_empty_list_on_empty_folder(
        self, organisation_a_folder
    ):
        pl = QgisProjectCollection(organisation_a_folder.name).projects()
        assert len(pl) == 0

    def test_qgis_project_collection_return_list_when_project_file(
        self, orga_a_project_file
    ):
    ...
    def test_qgis_project_collection_filters_as_expected(
        ...
        pl = pc.projects_filtered(
            {orga_a_project_file.relative_to(orga_a_project_file.parent)}
        )
        assert len(pl) == 0
        pl = pc.projects_filtered(
            {orga_b_project_file.relative_to(orga_b_project_file.parent)}
        )
        assert len(pl) == 1
    ...
    def test_qgis_project_assembles_integration_root_correctly(
        self, orga_a_project_file
    ):
    ...
    def test_qgis_project_provides_correct_config_path_from_root(
        self, orga_a_project_file
    ):
        p = QgisProject(orga_a_project_file, orga_a_project_file.parent.name)
        assert (
            str(p.config_path_from_root)
            == str(orga_a_project_file).replace(
                str(orga_a_project_file.parent.parent) + "/", ""
            )
            + "."
            + p.config_format
    ...
    def test_qgis_project_provides_correct_config_path_from_orga(
        self, orga_a_project_file
    ):
        p = QgisProject(orga_a_project_file, orga_a_project_file.parent.name)
        assert (
    ...
    def test_qgis_project_hash_is_none_on_nonexisting_config(
        self, orga_a_project_random_file
    ):
        orga_a_project_file = orga_a_project_random_file.with_suffix("")
        p = QgisProject(orga_a_project_file, orga_a_project_file.parent.name)
    ...
    def test_qgis_project_delivers_hash_on_existing_config(
        self, orga_b_project_config_file
    ):
        orga_a_project_file = orga_b_project_config_file.with_suffix("")
        p = QgisProject(orga_a_project_file, orga_a_project_file.parent.name)
