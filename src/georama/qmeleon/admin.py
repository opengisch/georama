...
class QgisProject:
    ...
    def config_path(self) -> str:
        return os.path.join(self.parent.parent.path, self.parent.name, self.qualified_config_name)

    @property
    def project_path(self) -> str:
        return os.path.join(self.parent.parent.path, self.parent.name, self.qualified_project_name)
...
class QgisProjectGroup:
    ...
    name: str
    projects: list[QgisProject] = field(
...
class QgisProjectFileStructure:
    path: str
    groups: list[QgisProjectGroup] = field(
        default_factory=list
    )
...
class ProjectAdmin(admin.ModelAdmin):
    ...
    def qgis_projects(self, request: HttpRequest):
        ...
        return TemplateResponse(
            request,
            "admin/qmeleon/project/qgis_projects.html",
            context
        )
...
class DataSetAdmin(admin.ModelAdmin):
    ...
    def source_detail(self, obj):
        ...
        for key in obj.source:
            snippet_parts.append(f'<li><label>{key}</label> → <span class="badge badge-secondary">{obj.source[key]}</span></li>')
    ...
    def crs_detail(self, obj):
        snippet_parts = ['<ul>']
        for key in obj.crs:
            snippet_parts.append(f'<li><label>{key}</label> → <span class="badge badge-secondary">{obj.crs[key]}</span></li>')
        snippet_parts.append('</ul>')
        return mark_safe(''.join(snippet_parts))
...
class VectorDataSetAdmin(DataSetAdmin):
    ...
    def field_count(self, obj):
        return mark_safe('<span class="badge badge-secondary">{}</span>'.format(
            obj.fields.count()
        ))
