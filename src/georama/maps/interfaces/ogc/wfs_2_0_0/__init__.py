from georama.maps.interfaces.ogc.wfs_2_0_0.abstract_1 import Abstract1
from georama.maps.interfaces.ogc.wfs_2_0_0.abstract_2 import Abstract2
from georama.maps.interfaces.ogc.wfs_2_0_0.abstract_adhoc_query_expression import (
    AbstractAdhocQueryExpression,
)
from georama.maps.interfaces.ogc.wfs_2_0_0.abstract_adhoc_query_expression_type import (
    AbstractAdhocQueryExpressionType,
)
from georama.maps.interfaces.ogc.wfs_2_0_0.abstract_id_type import AbstractIdType
from georama.maps.interfaces.ogc.wfs_2_0_0.abstract_meta_data import AbstractMetaData
from georama.maps.interfaces.ogc.wfs_2_0_0.abstract_projection_clause import (
    AbstractProjectionClause,
)
from georama.maps.interfaces.ogc.wfs_2_0_0.abstract_projection_clause_type import (
    AbstractProjectionClauseType,
)
from georama.maps.interfaces.ogc.wfs_2_0_0.abstract_query_expression import (
    AbstractQueryExpression,
)
from georama.maps.interfaces.ogc.wfs_2_0_0.abstract_query_expression_type import (
    AbstractQueryExpressionType,
)
from georama.maps.interfaces.ogc.wfs_2_0_0.abstract_reference_base import (
    AbstractReferenceBase,
)
from georama.maps.interfaces.ogc.wfs_2_0_0.abstract_reference_base_type import (
    AbstractReferenceBaseType,
)
from georama.maps.interfaces.ogc.wfs_2_0_0.abstract_selection_clause import (
    AbstractSelectionClause,
)
from georama.maps.interfaces.ogc.wfs_2_0_0.abstract_selection_clause_type import (
    AbstractSelectionClauseType,
)
from georama.maps.interfaces.ogc.wfs_2_0_0.abstract_sorting_clause import (
    AbstractSortingClause,
)
from georama.maps.interfaces.ogc.wfs_2_0_0.abstract_sorting_clause_type import (
    AbstractSortingClauseType,
)
from georama.maps.interfaces.ogc.wfs_2_0_0.abstract_transaction_action import (
    AbstractTransactionAction,
)
from georama.maps.interfaces.ogc.wfs_2_0_0.abstract_transaction_action_type import (
    AbstractTransactionActionType,
)
from georama.maps.interfaces.ogc.wfs_2_0_0.accept_formats_type import AcceptFormatsType
from georama.maps.interfaces.ogc.wfs_2_0_0.accept_versions_type import (
    AcceptVersionsType,
)
from georama.maps.interfaces.ogc.wfs_2_0_0.access_constraints import AccessConstraints
from georama.maps.interfaces.ogc.wfs_2_0_0.action_results_type import ActionResultsType
from georama.maps.interfaces.ogc.wfs_2_0_0.actuate_type import ActuateType
from georama.maps.interfaces.ogc.wfs_2_0_0.additional_operators_type import (
    AdditionalOperatorsType,
)
from georama.maps.interfaces.ogc.wfs_2_0_0.address_type import AddressType
from georama.maps.interfaces.ogc.wfs_2_0_0.after import After
from georama.maps.interfaces.ogc.wfs_2_0_0.all_some_type import AllSomeType
from georama.maps.interfaces.ogc.wfs_2_0_0.allowed_values import AllowedValues
from georama.maps.interfaces.ogc.wfs_2_0_0.any_interacts import AnyInteracts
from georama.maps.interfaces.ogc.wfs_2_0_0.any_value import AnyValue
from georama.maps.interfaces.ogc.wfs_2_0_0.arc import Arc
from georama.maps.interfaces.ogc.wfs_2_0_0.arc_type import ArcType
from georama.maps.interfaces.ogc.wfs_2_0_0.argument_type import ArgumentType
from georama.maps.interfaces.ogc.wfs_2_0_0.arguments_type import ArgumentsType
from georama.maps.interfaces.ogc.wfs_2_0_0.available_crs import AvailableCrs
from georama.maps.interfaces.ogc.wfs_2_0_0.available_function_type import (
    AvailableFunctionType,
)
from georama.maps.interfaces.ogc.wfs_2_0_0.available_functions_type import (
    AvailableFunctionsType,
)
from georama.maps.interfaces.ogc.wfs_2_0_0.base_request_type import BaseRequestType
from georama.maps.interfaces.ogc.wfs_2_0_0.basic_identification_type import (
    BasicIdentificationType,
)
from georama.maps.interfaces.ogc.wfs_2_0_0.bbox import Bbox
from georama.maps.interfaces.ogc.wfs_2_0_0.bboxtype import Bboxtype
from georama.maps.interfaces.ogc.wfs_2_0_0.before import Before
from georama.maps.interfaces.ogc.wfs_2_0_0.begins import Begins
from georama.maps.interfaces.ogc.wfs_2_0_0.begun_by import BegunBy
from georama.maps.interfaces.ogc.wfs_2_0_0.beyond import Beyond
from georama.maps.interfaces.ogc.wfs_2_0_0.binary_comparison_op_type import (
    BinaryComparisonOpType,
)
from georama.maps.interfaces.ogc.wfs_2_0_0.binary_logic_op_type import (
    And,
    BinaryLogicOpType,
    Not,
    Or,
    UnaryLogicOpType,
)
from georama.maps.interfaces.ogc.wfs_2_0_0.binary_spatial_op_type import (
    BinarySpatialOpType,
)
from georama.maps.interfaces.ogc.wfs_2_0_0.binary_temporal_op_type import (
    BinaryTemporalOpType,
)
from georama.maps.interfaces.ogc.wfs_2_0_0.bounded_by import BoundedBy
from georama.maps.interfaces.ogc.wfs_2_0_0.bounding_box import BoundingBox
from georama.maps.interfaces.ogc.wfs_2_0_0.bounding_box_type import BoundingBoxType
from georama.maps.interfaces.ogc.wfs_2_0_0.capabilities_base_type import (
    CapabilitiesBaseType,
)
from georama.maps.interfaces.ogc.wfs_2_0_0.code_type import CodeType
from georama.maps.interfaces.ogc.wfs_2_0_0.comparison_operator_name_type_value import (
    ComparisonOperatorNameTypeValue,
)
from georama.maps.interfaces.ogc.wfs_2_0_0.comparison_operator_type import (
    ComparisonOperatorType,
)
from georama.maps.interfaces.ogc.wfs_2_0_0.comparison_operators_type import (
    ComparisonOperatorsType,
)
from georama.maps.interfaces.ogc.wfs_2_0_0.comparison_ops import ComparisonOps
from georama.maps.interfaces.ogc.wfs_2_0_0.comparison_ops_type import ComparisonOpsType
from georama.maps.interfaces.ogc.wfs_2_0_0.conformance_type import ConformanceType
from georama.maps.interfaces.ogc.wfs_2_0_0.contact_info import ContactInfo
from georama.maps.interfaces.ogc.wfs_2_0_0.contact_type import ContactType
from georama.maps.interfaces.ogc.wfs_2_0_0.contains import Contains
from georama.maps.interfaces.ogc.wfs_2_0_0.contents_base_type import ContentsBaseType
from georama.maps.interfaces.ogc.wfs_2_0_0.create_stored_query import CreateStoredQuery
from georama.maps.interfaces.ogc.wfs_2_0_0.create_stored_query_response import (
    CreateStoredQueryResponse,
)
from georama.maps.interfaces.ogc.wfs_2_0_0.create_stored_query_response_type import (
    CreateStoredQueryResponseType,
)
from georama.maps.interfaces.ogc.wfs_2_0_0.create_stored_query_type import (
    CreateStoredQueryType,
)
from georama.maps.interfaces.ogc.wfs_2_0_0.created_or_modified_feature_type import (
    CreatedOrModifiedFeatureType,
)
from georama.maps.interfaces.ogc.wfs_2_0_0.crosses import Crosses
from georama.maps.interfaces.ogc.wfs_2_0_0.data_type import DataType
from georama.maps.interfaces.ogc.wfs_2_0_0.dataset_description_summary_base_type import (
    DatasetDescriptionSummary,
    DatasetDescriptionSummaryBaseType,
)
from georama.maps.interfaces.ogc.wfs_2_0_0.dcp import Dcp
from georama.maps.interfaces.ogc.wfs_2_0_0.default_crs import DefaultCrs
from georama.maps.interfaces.ogc.wfs_2_0_0.default_value import DefaultValue
from georama.maps.interfaces.ogc.wfs_2_0_0.delete import Delete
from georama.maps.interfaces.ogc.wfs_2_0_0.delete_type import DeleteType
from georama.maps.interfaces.ogc.wfs_2_0_0.describe_feature_type import (
    DescribeFeatureType,
)
from georama.maps.interfaces.ogc.wfs_2_0_0.describe_feature_type_type import (
    DescribeFeatureTypeType,
)
from georama.maps.interfaces.ogc.wfs_2_0_0.describe_stored_queries import (
    DescribeStoredQueries,
)
from georama.maps.interfaces.ogc.wfs_2_0_0.describe_stored_queries_response import (
    DescribeStoredQueriesResponse,
)
from georama.maps.interfaces.ogc.wfs_2_0_0.describe_stored_queries_response_type import (
    DescribeStoredQueriesResponseType,
)
from georama.maps.interfaces.ogc.wfs_2_0_0.describe_stored_queries_type import (
    DescribeStoredQueriesType,
)
from georama.maps.interfaces.ogc.wfs_2_0_0.description_type import DescriptionType
from georama.maps.interfaces.ogc.wfs_2_0_0.disjoint import Disjoint
from georama.maps.interfaces.ogc.wfs_2_0_0.distance_buffer_type import (
    DistanceBufferType,
)
from georama.maps.interfaces.ogc.wfs_2_0_0.domain_metadata_type import (
    DomainMetadataType,
)
from georama.maps.interfaces.ogc.wfs_2_0_0.domain_type import DomainType
from georama.maps.interfaces.ogc.wfs_2_0_0.drop_stored_query import DropStoredQuery
from georama.maps.interfaces.ogc.wfs_2_0_0.drop_stored_query_response import (
    DropStoredQueryResponse,
)
from georama.maps.interfaces.ogc.wfs_2_0_0.during import During
from georama.maps.interfaces.ogc.wfs_2_0_0.dwithin import Dwithin
from georama.maps.interfaces.ogc.wfs_2_0_0.element import Element
from georama.maps.interfaces.ogc.wfs_2_0_0.element_type import ElementType
from georama.maps.interfaces.ogc.wfs_2_0_0.empty_type import EmptyType
from georama.maps.interfaces.ogc.wfs_2_0_0.ended_by import EndedBy
from georama.maps.interfaces.ogc.wfs_2_0_0.ends import Ends
from georama.maps.interfaces.ogc.wfs_2_0_0.envelope_property_type import (
    EnvelopePropertyType,
)
from georama.maps.interfaces.ogc.wfs_2_0_0.equals import Equals
from georama.maps.interfaces.ogc.wfs_2_0_0.exception import Exception
from georama.maps.interfaces.ogc.wfs_2_0_0.exception_report import ExceptionReport
from georama.maps.interfaces.ogc.wfs_2_0_0.exception_type import ExceptionType
from georama.maps.interfaces.ogc.wfs_2_0_0.execution_status_type import (
    ExecutionStatusType,
)
from georama.maps.interfaces.ogc.wfs_2_0_0.expression import Expression
from georama.maps.interfaces.ogc.wfs_2_0_0.extended import Extended
from georama.maps.interfaces.ogc.wfs_2_0_0.extended_capabilities import (
    ExtendedCapabilities,
)
from georama.maps.interfaces.ogc.wfs_2_0_0.extended_capabilities_type import (
    ExtendedCapabilitiesType,
)
from georama.maps.interfaces.ogc.wfs_2_0_0.extended_description_type import (
    ExtendedDescriptionType,
)
from georama.maps.interfaces.ogc.wfs_2_0_0.extension_operator_type import (
    ExtensionOperatorType,
)
from georama.maps.interfaces.ogc.wfs_2_0_0.extension_ops import ExtensionOps
from georama.maps.interfaces.ogc.wfs_2_0_0.extension_ops_type import ExtensionOpsType
from georama.maps.interfaces.ogc.wfs_2_0_0.feature_type_list import FeatureTypeList
from georama.maps.interfaces.ogc.wfs_2_0_0.feature_type_list_type import (
    FeatureTypeListType,
)
from georama.maps.interfaces.ogc.wfs_2_0_0.feature_type_type import FeatureTypeType
from georama.maps.interfaces.ogc.wfs_2_0_0.feature_type_type_no_crs import (
    FeatureTypeTypeNoCrs,
)
from georama.maps.interfaces.ogc.wfs_2_0_0.features_locked_type import (
    FeaturesLockedType,
)
from georama.maps.interfaces.ogc.wfs_2_0_0.features_not_locked_type import (
    FeaturesNotLockedType,
)
from georama.maps.interfaces.ogc.wfs_2_0_0.fees import Fees
from georama.maps.interfaces.ogc.wfs_2_0_0.filter import Filter
from georama.maps.interfaces.ogc.wfs_2_0_0.filter_capabilities import FilterCapabilities
from georama.maps.interfaces.ogc.wfs_2_0_0.filter_type import FilterType
from georama.maps.interfaces.ogc.wfs_2_0_0.function_type import Function, FunctionType
from georama.maps.interfaces.ogc.wfs_2_0_0.geometry_operands_type import (
    GeometryOperandsType,
)
from georama.maps.interfaces.ogc.wfs_2_0_0.geometry_operands_type_geometry_operand import (
    GeometryOperandsTypeGeometryOperand,
)
from georama.maps.interfaces.ogc.wfs_2_0_0.get import Get
from georama.maps.interfaces.ogc.wfs_2_0_0.get_capabilities_1 import GetCapabilities1
from georama.maps.interfaces.ogc.wfs_2_0_0.get_capabilities_2 import GetCapabilities2
from georama.maps.interfaces.ogc.wfs_2_0_0.get_capabilities_type_1 import (
    GetCapabilitiesType1,
)
from georama.maps.interfaces.ogc.wfs_2_0_0.get_capabilities_type_2 import (
    GetCapabilitiesType2,
)
from georama.maps.interfaces.ogc.wfs_2_0_0.get_feature import GetFeature
from georama.maps.interfaces.ogc.wfs_2_0_0.get_feature_type import GetFeatureType
from georama.maps.interfaces.ogc.wfs_2_0_0.get_feature_with_lock import (
    GetFeatureWithLock,
)
from georama.maps.interfaces.ogc.wfs_2_0_0.get_feature_with_lock_type import (
    GetFeatureWithLockType,
)
from georama.maps.interfaces.ogc.wfs_2_0_0.get_property_value import GetPropertyValue
from georama.maps.interfaces.ogc.wfs_2_0_0.get_property_value_type import (
    GetPropertyValueType,
)
from georama.maps.interfaces.ogc.wfs_2_0_0.get_resource_by_id import GetResourceById
from georama.maps.interfaces.ogc.wfs_2_0_0.get_resource_by_id_type import (
    GetResourceByIdType,
)
from georama.maps.interfaces.ogc.wfs_2_0_0.http import Http
from georama.maps.interfaces.ogc.wfs_2_0_0.id import Id
from georama.maps.interfaces.ogc.wfs_2_0_0.id_capabilities_type import (
    IdCapabilitiesType,
)
from georama.maps.interfaces.ogc.wfs_2_0_0.identification_type import IdentificationType
from georama.maps.interfaces.ogc.wfs_2_0_0.identifier import Identifier
from georama.maps.interfaces.ogc.wfs_2_0_0.individual_name import IndividualName
from georama.maps.interfaces.ogc.wfs_2_0_0.input_data import InputData
from georama.maps.interfaces.ogc.wfs_2_0_0.insert import Insert
from georama.maps.interfaces.ogc.wfs_2_0_0.insert_type import InsertType
from georama.maps.interfaces.ogc.wfs_2_0_0.intersects import Intersects
from georama.maps.interfaces.ogc.wfs_2_0_0.keywords import Keywords
from georama.maps.interfaces.ogc.wfs_2_0_0.keywords_type import KeywordsType
from georama.maps.interfaces.ogc.wfs_2_0_0.lang_value import LangValue
from georama.maps.interfaces.ogc.wfs_2_0_0.language import Language
from georama.maps.interfaces.ogc.wfs_2_0_0.language_string_type import (
    LanguageStringType,
)
from georama.maps.interfaces.ogc.wfs_2_0_0.list_stored_queries import ListStoredQueries
from georama.maps.interfaces.ogc.wfs_2_0_0.list_stored_queries_response import (
    ListStoredQueriesResponse,
)
from georama.maps.interfaces.ogc.wfs_2_0_0.list_stored_queries_response_type import (
    ListStoredQueriesResponseType,
)
from georama.maps.interfaces.ogc.wfs_2_0_0.list_stored_queries_type import (
    ListStoredQueriesType,
)
from georama.maps.interfaces.ogc.wfs_2_0_0.literal import Literal
from georama.maps.interfaces.ogc.wfs_2_0_0.literal_type import LiteralType
from georama.maps.interfaces.ogc.wfs_2_0_0.locator import Locator
from georama.maps.interfaces.ogc.wfs_2_0_0.locator_type import LocatorType
from georama.maps.interfaces.ogc.wfs_2_0_0.lock_feature import LockFeature
from georama.maps.interfaces.ogc.wfs_2_0_0.lock_feature_response import (
    LockFeatureResponse,
)
from georama.maps.interfaces.ogc.wfs_2_0_0.lock_feature_response_type import (
    LockFeatureResponseType,
)
from georama.maps.interfaces.ogc.wfs_2_0_0.lock_feature_type import LockFeatureType
from georama.maps.interfaces.ogc.wfs_2_0_0.logic_ops import LogicOps
from georama.maps.interfaces.ogc.wfs_2_0_0.logic_ops_type import LogicOpsType
from georama.maps.interfaces.ogc.wfs_2_0_0.logical_operators import LogicalOperators
from georama.maps.interfaces.ogc.wfs_2_0_0.lower_boundary_type import LowerBoundaryType
from georama.maps.interfaces.ogc.wfs_2_0_0.manifest import Manifest
from georama.maps.interfaces.ogc.wfs_2_0_0.manifest_type import ManifestType
from georama.maps.interfaces.ogc.wfs_2_0_0.match_action_type import MatchActionType
from georama.maps.interfaces.ogc.wfs_2_0_0.maximum_value import MaximumValue
from georama.maps.interfaces.ogc.wfs_2_0_0.meaning import Meaning
from georama.maps.interfaces.ogc.wfs_2_0_0.measure_type import MeasureType
from georama.maps.interfaces.ogc.wfs_2_0_0.meets import Meets
from georama.maps.interfaces.ogc.wfs_2_0_0.member_property_type import (
    AdditionalObjects,
    AdditionalValues,
    FeatureCollection,
    FeatureCollectionType,
    Member,
    MemberPropertyType,
    SimpleFeatureCollection,
    SimpleFeatureCollectionType,
    Tuple,
    TupleType,
    ValueCollection,
    ValueCollectionType,
)
from georama.maps.interfaces.ogc.wfs_2_0_0.met_by import MetBy
from georama.maps.interfaces.ogc.wfs_2_0_0.metadata import Metadata
from georama.maps.interfaces.ogc.wfs_2_0_0.metadata_type import MetadataType
from georama.maps.interfaces.ogc.wfs_2_0_0.metadata_urltype import MetadataUrltype
from georama.maps.interfaces.ogc.wfs_2_0_0.minimum_value import MinimumValue
from georama.maps.interfaces.ogc.wfs_2_0_0.native import Native
from georama.maps.interfaces.ogc.wfs_2_0_0.native_type import NativeType
from georama.maps.interfaces.ogc.wfs_2_0_0.no_values import NoValues
from georama.maps.interfaces.ogc.wfs_2_0_0.non_negative_integer_or_unknown_value import (
    NonNegativeIntegerOrUnknownValue,
)
from georama.maps.interfaces.ogc.wfs_2_0_0.online_resource_type import (
    OnlineResourceType,
)
from georama.maps.interfaces.ogc.wfs_2_0_0.operation import Operation
from georama.maps.interfaces.ogc.wfs_2_0_0.operation_response import OperationResponse
from georama.maps.interfaces.ogc.wfs_2_0_0.operations_metadata import OperationsMetadata
from georama.maps.interfaces.ogc.wfs_2_0_0.organisation_name import OrganisationName
from georama.maps.interfaces.ogc.wfs_2_0_0.other_crs import OtherCrs
from georama.maps.interfaces.ogc.wfs_2_0_0.other_source import OtherSource
from georama.maps.interfaces.ogc.wfs_2_0_0.output_format import OutputFormat
from georama.maps.interfaces.ogc.wfs_2_0_0.output_format_list_type import (
    OutputFormatListType,
)
from georama.maps.interfaces.ogc.wfs_2_0_0.overlapped_by import OverlappedBy
from georama.maps.interfaces.ogc.wfs_2_0_0.overlaps import Overlaps
from georama.maps.interfaces.ogc.wfs_2_0_0.parameter_expression_type import (
    ParameterExpressionType,
)
from georama.maps.interfaces.ogc.wfs_2_0_0.parameter_type import ParameterType
from georama.maps.interfaces.ogc.wfs_2_0_0.point_of_contact import PointOfContact
from georama.maps.interfaces.ogc.wfs_2_0_0.position_name import PositionName
from georama.maps.interfaces.ogc.wfs_2_0_0.post import Post
from georama.maps.interfaces.ogc.wfs_2_0_0.property import Property
from georama.maps.interfaces.ogc.wfs_2_0_0.property_is_between import PropertyIsBetween
from georama.maps.interfaces.ogc.wfs_2_0_0.property_is_between_type import (
    PropertyIsBetweenType,
)
from georama.maps.interfaces.ogc.wfs_2_0_0.property_is_equal_to import PropertyIsEqualTo
from georama.maps.interfaces.ogc.wfs_2_0_0.property_is_greater_than import (
    PropertyIsGreaterThan,
)
from georama.maps.interfaces.ogc.wfs_2_0_0.property_is_greater_than_or_equal_to import (
    PropertyIsGreaterThanOrEqualTo,
)
from georama.maps.interfaces.ogc.wfs_2_0_0.property_is_less_than import (
    PropertyIsLessThan,
)
from georama.maps.interfaces.ogc.wfs_2_0_0.property_is_less_than_or_equal_to import (
    PropertyIsLessThanOrEqualTo,
)
from georama.maps.interfaces.ogc.wfs_2_0_0.property_is_like import PropertyIsLike
from georama.maps.interfaces.ogc.wfs_2_0_0.property_is_like_type import (
    PropertyIsLikeType,
)
from georama.maps.interfaces.ogc.wfs_2_0_0.property_is_nil import PropertyIsNil
from georama.maps.interfaces.ogc.wfs_2_0_0.property_is_nil_type import PropertyIsNilType
from georama.maps.interfaces.ogc.wfs_2_0_0.property_is_not_equal_to import (
    PropertyIsNotEqualTo,
)
from georama.maps.interfaces.ogc.wfs_2_0_0.property_is_null import PropertyIsNull
from georama.maps.interfaces.ogc.wfs_2_0_0.property_is_null_type import (
    PropertyIsNullType,
)
from georama.maps.interfaces.ogc.wfs_2_0_0.property_name import PropertyName
from georama.maps.interfaces.ogc.wfs_2_0_0.property_type import PropertyType
from georama.maps.interfaces.ogc.wfs_2_0_0.property_type_value_reference import (
    PropertyTypeValueReference,
)
from georama.maps.interfaces.ogc.wfs_2_0_0.query import Query
from georama.maps.interfaces.ogc.wfs_2_0_0.query_expression_text_type import (
    QueryExpressionTextType,
)
from georama.maps.interfaces.ogc.wfs_2_0_0.query_type import QueryType
from georama.maps.interfaces.ogc.wfs_2_0_0.range import Range
from georama.maps.interfaces.ogc.wfs_2_0_0.range_closure_value import RangeClosureValue
from georama.maps.interfaces.ogc.wfs_2_0_0.range_type import RangeType
from georama.maps.interfaces.ogc.wfs_2_0_0.reference import Reference
from georama.maps.interfaces.ogc.wfs_2_0_0.reference_group import ReferenceGroup
from georama.maps.interfaces.ogc.wfs_2_0_0.reference_group_type import (
    ReferenceGroupType,
)
from georama.maps.interfaces.ogc.wfs_2_0_0.reference_system import ReferenceSystem
from georama.maps.interfaces.ogc.wfs_2_0_0.reference_type import ReferenceType
from georama.maps.interfaces.ogc.wfs_2_0_0.replace import Replace
from georama.maps.interfaces.ogc.wfs_2_0_0.replace_type import ReplaceType
from georama.maps.interfaces.ogc.wfs_2_0_0.request_method_type import RequestMethodType
from georama.maps.interfaces.ogc.wfs_2_0_0.resolve_value_type import ResolveValueType
from georama.maps.interfaces.ogc.wfs_2_0_0.resource import Resource
from georama.maps.interfaces.ogc.wfs_2_0_0.resource_abstract import ResourceAbstract
from georama.maps.interfaces.ogc.wfs_2_0_0.resource_id import ResourceId
from georama.maps.interfaces.ogc.wfs_2_0_0.resource_id_type import ResourceIdType
from georama.maps.interfaces.ogc.wfs_2_0_0.resource_identifier_type import (
    ResourceIdentifierType,
)
from georama.maps.interfaces.ogc.wfs_2_0_0.resource_type import ResourceType
from georama.maps.interfaces.ogc.wfs_2_0_0.responsible_party_subset_type import (
    ResponsiblePartySubsetType,
)
from georama.maps.interfaces.ogc.wfs_2_0_0.responsible_party_type import (
    ResponsiblePartyType,
)
from georama.maps.interfaces.ogc.wfs_2_0_0.result_type_type import ResultTypeType
from georama.maps.interfaces.ogc.wfs_2_0_0.role import Role
from georama.maps.interfaces.ogc.wfs_2_0_0.scalar_capabilities_type import (
    ScalarCapabilitiesType,
)
from georama.maps.interfaces.ogc.wfs_2_0_0.sections_type import SectionsType
from georama.maps.interfaces.ogc.wfs_2_0_0.service_identification import (
    ServiceIdentification,
)
from georama.maps.interfaces.ogc.wfs_2_0_0.service_provider import ServiceProvider
from georama.maps.interfaces.ogc.wfs_2_0_0.service_reference import ServiceReference
from georama.maps.interfaces.ogc.wfs_2_0_0.service_reference_type import (
    ServiceReferenceType,
)
from georama.maps.interfaces.ogc.wfs_2_0_0.show_type import ShowType
from georama.maps.interfaces.ogc.wfs_2_0_0.simple import Simple
from georama.maps.interfaces.ogc.wfs_2_0_0.sort_by import SortBy
from georama.maps.interfaces.ogc.wfs_2_0_0.sort_by_type import SortByType
from georama.maps.interfaces.ogc.wfs_2_0_0.sort_order_type import SortOrderType
from georama.maps.interfaces.ogc.wfs_2_0_0.sort_property_type import SortPropertyType
from georama.maps.interfaces.ogc.wfs_2_0_0.spacing import Spacing
from georama.maps.interfaces.ogc.wfs_2_0_0.spatial_capabilities_type import (
    SpatialCapabilitiesType,
)
from georama.maps.interfaces.ogc.wfs_2_0_0.spatial_operator_name_type_value import (
    SpatialOperatorNameTypeValue,
)
from georama.maps.interfaces.ogc.wfs_2_0_0.spatial_operator_type import (
    SpatialOperatorType,
)
from georama.maps.interfaces.ogc.wfs_2_0_0.spatial_operators_type import (
    SpatialOperatorsType,
)
from georama.maps.interfaces.ogc.wfs_2_0_0.spatial_ops import SpatialOps
from georama.maps.interfaces.ogc.wfs_2_0_0.spatial_ops_type import SpatialOpsType
from georama.maps.interfaces.ogc.wfs_2_0_0.star_string_type import StarStringType
from georama.maps.interfaces.ogc.wfs_2_0_0.state_value_type_value import (
    StateValueTypeValue,
)
from georama.maps.interfaces.ogc.wfs_2_0_0.stored_query import StoredQuery
from georama.maps.interfaces.ogc.wfs_2_0_0.stored_query_description_type import (
    StoredQueryDescriptionType,
)
from georama.maps.interfaces.ogc.wfs_2_0_0.stored_query_list_item_type import (
    StoredQueryListItemType,
)
from georama.maps.interfaces.ogc.wfs_2_0_0.stored_query_type import StoredQueryType
from georama.maps.interfaces.ogc.wfs_2_0_0.supported_crs import SupportedCrs
from georama.maps.interfaces.ogc.wfs_2_0_0.tcontains import Tcontains
from georama.maps.interfaces.ogc.wfs_2_0_0.telephone_type import TelephoneType
from georama.maps.interfaces.ogc.wfs_2_0_0.temporal_capabilities_type import (
    TemporalCapabilitiesType,
)
from georama.maps.interfaces.ogc.wfs_2_0_0.temporal_operands_type import (
    TemporalOperandsType,
)
from georama.maps.interfaces.ogc.wfs_2_0_0.temporal_operands_type_temporal_operand import (
    TemporalOperandsTypeTemporalOperand,
)
from georama.maps.interfaces.ogc.wfs_2_0_0.temporal_operator_name_type_value import (
    TemporalOperatorNameTypeValue,
)
from georama.maps.interfaces.ogc.wfs_2_0_0.temporal_operator_type import (
    TemporalOperatorType,
)
from georama.maps.interfaces.ogc.wfs_2_0_0.temporal_operators_type import (
    TemporalOperatorsType,
)
from georama.maps.interfaces.ogc.wfs_2_0_0.temporal_ops import TemporalOps
from georama.maps.interfaces.ogc.wfs_2_0_0.temporal_ops_type import TemporalOpsType
from georama.maps.interfaces.ogc.wfs_2_0_0.tequals import Tequals
from georama.maps.interfaces.ogc.wfs_2_0_0.title_1 import Title1
from georama.maps.interfaces.ogc.wfs_2_0_0.title_2 import Title2
from georama.maps.interfaces.ogc.wfs_2_0_0.title_3 import Title3
from georama.maps.interfaces.ogc.wfs_2_0_0.title_elt_type import TitleEltType
from georama.maps.interfaces.ogc.wfs_2_0_0.touches import Touches
from georama.maps.interfaces.ogc.wfs_2_0_0.toverlaps import Toverlaps
from georama.maps.interfaces.ogc.wfs_2_0_0.transaction import Transaction
from georama.maps.interfaces.ogc.wfs_2_0_0.transaction_response import (
    TransactionResponse,
)
from georama.maps.interfaces.ogc.wfs_2_0_0.transaction_response_type import (
    TransactionResponseType,
)
from georama.maps.interfaces.ogc.wfs_2_0_0.transaction_summary_type import (
    TransactionSummaryType,
)
from georama.maps.interfaces.ogc.wfs_2_0_0.transaction_type import TransactionType
from georama.maps.interfaces.ogc.wfs_2_0_0.truncated_response import TruncatedResponse
from georama.maps.interfaces.ogc.wfs_2_0_0.type_type import TypeType
from georama.maps.interfaces.ogc.wfs_2_0_0.un_named_domain_type import UnNamedDomainType
from georama.maps.interfaces.ogc.wfs_2_0_0.uom import Uom
from georama.maps.interfaces.ogc.wfs_2_0_0.update import Update
from georama.maps.interfaces.ogc.wfs_2_0_0.update_action_type import UpdateActionType
from georama.maps.interfaces.ogc.wfs_2_0_0.update_type import UpdateType
from georama.maps.interfaces.ogc.wfs_2_0_0.upper_boundary_type import UpperBoundaryType
from georama.maps.interfaces.ogc.wfs_2_0_0.value_1 import Value1
from georama.maps.interfaces.ogc.wfs_2_0_0.value_2 import Value2
from georama.maps.interfaces.ogc.wfs_2_0_0.value_list import ValueList
from georama.maps.interfaces.ogc.wfs_2_0_0.value_list_type import ValueListType
from georama.maps.interfaces.ogc.wfs_2_0_0.value_reference import ValueReference
from georama.maps.interfaces.ogc.wfs_2_0_0.value_type import ValueType
from georama.maps.interfaces.ogc.wfs_2_0_0.values_reference import ValuesReference
from georama.maps.interfaces.ogc.wfs_2_0_0.version_action_tokens import (
    VersionActionTokens,
)
from georama.maps.interfaces.ogc.wfs_2_0_0.wfs_capabilities import WfsCapabilities
from georama.maps.interfaces.ogc.wfs_2_0_0.wfs_capabilities_type import (
    WfsCapabilitiesType,
)
from georama.maps.interfaces.ogc.wfs_2_0_0.wfs_capabilities_type_wsdl import (
    WfsCapabilitiesTypeWsdl,
)
from georama.maps.interfaces.ogc.wfs_2_0_0.wgs84_bounding_box import Wgs84BoundingBox
from georama.maps.interfaces.ogc.wfs_2_0_0.wgs84_bounding_box_type import (
    Wgs84BoundingBoxType,
)
from georama.maps.interfaces.ogc.wfs_2_0_0.within import Within

__all__ = [
    "Abstract1",
    "Abstract2",
    "AbstractAdhocQueryExpression",
    "AbstractAdhocQueryExpressionType",
    "AbstractIdType",
    "AbstractMetaData",
    "AbstractProjectionClause",
    "AbstractProjectionClauseType",
    "AbstractQueryExpression",
    "AbstractQueryExpressionType",
    "AbstractReferenceBase",
    "AbstractReferenceBaseType",
    "AbstractSelectionClause",
    "AbstractSelectionClauseType",
    "AbstractSortingClause",
    "AbstractSortingClauseType",
    "AbstractTransactionAction",
    "AbstractTransactionActionType",
    "AcceptFormatsType",
    "AcceptVersionsType",
    "AccessConstraints",
    "ActionResultsType",
    "ActuateType",
    "AdditionalObjects",
    "AdditionalOperatorsType",
    "AdditionalValues",
    "AddressType",
    "After",
    "AllSomeType",
    "AllowedValues",
    "And",
    "AnyInteracts",
    "AnyValue",
    "Arc",
    "ArcType",
    "ArgumentType",
    "ArgumentsType",
    "AvailableCrs",
    "AvailableFunctionType",
    "AvailableFunctionsType",
    "BaseRequestType",
    "BasicIdentificationType",
    "Bbox",
    "Bboxtype",
    "Before",
    "Begins",
    "BegunBy",
    "Beyond",
    "BinaryComparisonOpType",
    "BinaryLogicOpType",
    "BinarySpatialOpType",
    "BinaryTemporalOpType",
    "BoundedBy",
    "BoundingBox",
    "BoundingBoxType",
    "CapabilitiesBaseType",
    "CodeType",
    "ComparisonOperatorNameTypeValue",
    "ComparisonOperatorType",
    "ComparisonOperatorsType",
    "ComparisonOps",
    "ComparisonOpsType",
    "ConformanceType",
    "ContactInfo",
    "ContactType",
    "Contains",
    "ContentsBaseType",
    "CreateStoredQuery",
    "CreateStoredQueryResponse",
    "CreateStoredQueryResponseType",
    "CreateStoredQueryType",
    "CreatedOrModifiedFeatureType",
    "Crosses",
    "DataType",
    "DatasetDescriptionSummary",
    "DatasetDescriptionSummaryBaseType",
    "Dcp",
    "DefaultCrs",
    "DefaultValue",
    "Delete",
    "DeleteType",
    "DescribeFeatureType",
    "DescribeFeatureTypeType",
    "DescribeStoredQueries",
    "DescribeStoredQueriesResponse",
    "DescribeStoredQueriesResponseType",
    "DescribeStoredQueriesType",
    "DescriptionType",
    "Disjoint",
    "DistanceBufferType",
    "DomainMetadataType",
    "DomainType",
    "DropStoredQuery",
    "DropStoredQueryResponse",
    "During",
    "Dwithin",
    "Element",
    "ElementType",
    "EmptyType",
    "EndedBy",
    "Ends",
    "EnvelopePropertyType",
    "Equals",
    "Exception",
    "ExceptionReport",
    "ExceptionType",
    "ExecutionStatusType",
    "Expression",
    "Extended",
    "ExtendedCapabilities",
    "ExtendedCapabilitiesType",
    "ExtendedDescriptionType",
    "ExtensionOperatorType",
    "ExtensionOps",
    "ExtensionOpsType",
    "FeatureCollection",
    "FeatureCollectionType",
    "FeatureTypeList",
    "FeatureTypeListType",
    "FeatureTypeType",
    "FeatureTypeTypeNoCrs",
    "FeaturesLockedType",
    "FeaturesNotLockedType",
    "Fees",
    "Filter",
    "FilterCapabilities",
    "FilterType",
    "Function",
    "FunctionType",
    "GeometryOperandsType",
    "GeometryOperandsTypeGeometryOperand",
    "Get",
    "GetCapabilities1",
    "GetCapabilities2",
    "GetCapabilitiesType1",
    "GetCapabilitiesType2",
    "GetFeature",
    "GetFeatureType",
    "GetFeatureWithLock",
    "GetFeatureWithLockType",
    "GetPropertyValue",
    "GetPropertyValueType",
    "GetResourceById",
    "GetResourceByIdType",
    "Http",
    "Id",
    "IdCapabilitiesType",
    "IdentificationType",
    "Identifier",
    "IndividualName",
    "InputData",
    "Insert",
    "InsertType",
    "Intersects",
    "Keywords",
    "KeywordsType",
    "LangValue",
    "Language",
    "LanguageStringType",
    "ListStoredQueries",
    "ListStoredQueriesResponse",
    "ListStoredQueriesResponseType",
    "ListStoredQueriesType",
    "Literal",
    "LiteralType",
    "Locator",
    "LocatorType",
    "LockFeature",
    "LockFeatureResponse",
    "LockFeatureResponseType",
    "LockFeatureType",
    "LogicOps",
    "LogicOpsType",
    "LogicalOperators",
    "LowerBoundaryType",
    "Manifest",
    "ManifestType",
    "MatchActionType",
    "MaximumValue",
    "Meaning",
    "MeasureType",
    "Meets",
    "Member",
    "MemberPropertyType",
    "MetBy",
    "Metadata",
    "MetadataType",
    "MetadataUrltype",
    "MinimumValue",
    "Native",
    "NativeType",
    "NoValues",
    "NonNegativeIntegerOrUnknownValue",
    "Not",
    "OnlineResourceType",
    "Operation",
    "OperationResponse",
    "OperationsMetadata",
    "Or",
    "OrganisationName",
    "OtherCrs",
    "OtherSource",
    "OutputFormat",
    "OutputFormatListType",
    "OverlappedBy",
    "Overlaps",
    "ParameterExpressionType",
    "ParameterType",
    "PointOfContact",
    "PositionName",
    "Post",
    "Property",
    "PropertyIsBetween",
    "PropertyIsBetweenType",
    "PropertyIsEqualTo",
    "PropertyIsGreaterThan",
    "PropertyIsGreaterThanOrEqualTo",
    "PropertyIsLessThan",
    "PropertyIsLessThanOrEqualTo",
    "PropertyIsLike",
    "PropertyIsLikeType",
    "PropertyIsNil",
    "PropertyIsNilType",
    "PropertyIsNotEqualTo",
    "PropertyIsNull",
    "PropertyIsNullType",
    "PropertyName",
    "PropertyType",
    "PropertyTypeValueReference",
    "Query",
    "QueryExpressionTextType",
    "QueryType",
    "Range",
    "RangeClosureValue",
    "RangeType",
    "Reference",
    "ReferenceGroup",
    "ReferenceGroupType",
    "ReferenceSystem",
    "ReferenceType",
    "Replace",
    "ReplaceType",
    "RequestMethodType",
    "ResolveValueType",
    "Resource",
    "ResourceAbstract",
    "ResourceId",
    "ResourceIdType",
    "ResourceIdentifierType",
    "ResourceType",
    "ResponsiblePartySubsetType",
    "ResponsiblePartyType",
    "ResultTypeType",
    "Role",
    "ScalarCapabilitiesType",
    "SectionsType",
    "ServiceIdentification",
    "ServiceProvider",
    "ServiceReference",
    "ServiceReferenceType",
    "ShowType",
    "Simple",
    "SimpleFeatureCollection",
    "SimpleFeatureCollectionType",
    "SortBy",
    "SortByType",
    "SortOrderType",
    "SortPropertyType",
    "Spacing",
    "SpatialCapabilitiesType",
    "SpatialOperatorNameTypeValue",
    "SpatialOperatorType",
    "SpatialOperatorsType",
    "SpatialOps",
    "SpatialOpsType",
    "StarStringType",
    "StateValueTypeValue",
    "StoredQuery",
    "StoredQueryDescriptionType",
    "StoredQueryListItemType",
    "StoredQueryType",
    "SupportedCrs",
    "Tcontains",
    "TelephoneType",
    "TemporalCapabilitiesType",
    "TemporalOperandsType",
    "TemporalOperandsTypeTemporalOperand",
    "TemporalOperatorNameTypeValue",
    "TemporalOperatorType",
    "TemporalOperatorsType",
    "TemporalOps",
    "TemporalOpsType",
    "Tequals",
    "Title1",
    "Title2",
    "Title3",
    "TitleEltType",
    "Touches",
    "Toverlaps",
    "Transaction",
    "TransactionResponse",
    "TransactionResponseType",
    "TransactionSummaryType",
    "TransactionType",
    "TruncatedResponse",
    "Tuple",
    "TupleType",
    "TypeType",
    "UnNamedDomainType",
    "UnaryLogicOpType",
    "Uom",
    "Update",
    "UpdateActionType",
    "UpdateType",
    "UpperBoundaryType",
    "Value1",
    "Value2",
    "ValueCollection",
    "ValueCollectionType",
    "ValueList",
    "ValueListType",
    "ValueReference",
    "ValueType",
    "ValuesReference",
    "VersionActionTokens",
    "WfsCapabilities",
    "WfsCapabilitiesType",
    "WfsCapabilitiesTypeWsdl",
    "Wgs84BoundingBox",
    "Wgs84BoundingBoxType",
    "Within",
]
