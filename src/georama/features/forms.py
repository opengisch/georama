...
class PublishedAsOgcApiFeaturesForm(forms.ModelForm):
    ...
    group_create_permission = forms.ModelMultipleChoiceField(
        required=False,
        queryset=None,
        widget=widgets.FilteredSelectMultiple(verbose_name="Group create permission", is_stacked=False)
    )
    group_update_permission = forms.ModelMultipleChoiceField(
        required=False,
        queryset=None,
        widget=widgets.FilteredSelectMultiple(verbose_name="Group update permission", is_stacked=False)
    )
    group_delete_permission = forms.ModelMultipleChoiceField(
        required=False,
        queryset=None,
        widget=widgets.FilteredSelectMultiple(verbose_name="Group delete permission", is_stacked=False)
    )
    ...
    user_create_permission = forms.ModelMultipleChoiceField(
        required=False,
        queryset=None,
        widget=widgets.FilteredSelectMultiple(verbose_name="User create permission", is_stacked=False)
    )
    user_update_permission = forms.ModelMultipleChoiceField(
        required=False,
        queryset=None,
        widget=widgets.FilteredSelectMultiple(verbose_name="User update permission", is_stacked=False)
    )
    user_delete_permission = forms.ModelMultipleChoiceField(
        required=False,
        queryset=None,
        widget=widgets.FilteredSelectMultiple(verbose_name="User delete permission", is_stacked=False)
    )
    ...
    def __init__(self, *args, **kwargs):
        ...
        for perm in permission_codenames:
            permission_keys = PublishedAsOgcApiFeaturesForm.get_first_match_partial_key(perm_fields, perm.split("_")[1])
        ...
        for field, perm in perm_fields.items():
            if field == "column_permission":
                ...
                if 'group' in field:
                    ...
                    self.fields[field].queryset = User.objects.all().exclude(is_superuser=True)
                    self.fields[field].initial = User.objects.filter(user_permissions__codename=perm).exclude(is_superuser=True)
                else:
