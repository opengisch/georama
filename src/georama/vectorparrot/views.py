...
from georama.vectorparrot.vectorparrot_config import Config
...
from georama.vectorparrot.config_openapi import config_openapi
from georama.vectorparrot.config_server import config_server
from georama.vectorparrot.models import PublishedAsOgcApiFeatures
from georama.vectorparrot.apps import appname
...
def collections(request: HttpRequest,
                collection_id: typing.Optional[str] = None) -> HttpResponse:
    """
...
def collection_schema(request: HttpRequest,
    ...
    :returns: Django HTTP Response
    """
    if PublishedAsOgcApiFeatures.objects.get(identifier=collection_id).has_read_permission(request.user, appname):
...
def handle_runtime_config(request: HttpRequest) -> tuple[dict, dict]:
    ...
    server_config['server']['url'] = f'http://{request.get_host()}/vectorparrot'
    ...
    for published_as in PublishedAsOgcApiFeatures.objects.all():
        if published_as.has_general_permission(request.user, appname):
            server_config['resources'][str(published_as.identifier)] = create_resource(published_as, request)
            for path in create_oapi_cfg(published_as):
                openapi_config['paths'].update(path)
    return server_config, openapi_config


def _to_django_response(headers: typing.Mapping, status_code: int,
...
def _feed_response(request: HttpRequest, api_definition: str,
                   *args, **kwargs) -> typing.Tuple[typing.Dict, int, str]:
...
def create_resource(published_as: PublishedAsOgcApiFeatures, request: HttpRequest) -> dict:
    editable = (
       published_as.has_update_permission(request.user, appname) or
       published_as.has_create_permission(request.user, appname) or
       published_as.has_delete_permission(request.user, appname)
    )
...
def admin_publish_as_oapif(request: HttpRequest, vector_dataset_id: str):
    ...
    published_as_oapi = PublishedAsOgcApiFeatures(
        dataset=vd
    )
    published_as_oapi.save()
    return redirect('admin:vectorparrot_publishedasogcapifeatures_changelist')
