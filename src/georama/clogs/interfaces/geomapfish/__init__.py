...
from georama.clogs.interfaces.geomapfish.themes_json_2_8.dataclasses import (
    ThemesJson, OgcServer, LinkedLayer, Attribute, Themes
)
from georama.clogs.interfaces.geomapfish.themes_json_2_8.parsers import CustomDictDecoder
    ...
    for ogc_key in themes_json_dict["ogcServers"]:
        ...
        if isinstance(themes_json_dict["ogcServers"][ogc_key]["attributes"], dict):
            for linked_layer_key in themes_json_dict["ogcServers"][ogc_key]["attributes"]:
                ...
                for attr_key in themes_json_dict["ogcServers"][ogc_key]["attributes"][linked_layer_key]:
                    attribute = decoder.decode(
                        themes_json_dict["ogcServers"][ogc_key]["attributes"][linked_layer_key][attr_key],
                        Attribute
                    )
